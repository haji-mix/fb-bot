...
module.exports["config"] = {
    ...
    info: "Interact with Qwen 2.5 Coder AI.",
    usage: "[prompt]",
    guide: "qcoder create a node js program?",
    cd: 6
};
...
module.exports["run"] = async ({
    ...
    const getResponse = async () => {
        return axios.post(apiUrl, {
            ...
            headers: {
                'Authorization': 'Bearer glhf_af2a03d60456a07c8171f57949ba3d41',
    ...
    while (attempts < maxRetries && !success) {
        try {
            ...
            if (attempts < maxRetries) {
                await answering.edit(font.monospace(`No response from Qwen 2.5 Coder. Retrying... (${attempts} of ${maxRetries} attempts)`));
                await new Promise(resolve => setTimeout(resolve, 1000 * attempts));
            } else {
                answering.edit(font.monospace("No response from Qwen 2.5 Coder. Please try again later: " + error.message));
                return;
            }
        }
    }
    ...
    if (success) {
        ...
        const message = font.bold(" 🤖 | Qwen2.5-Coder-32B-Instruct".toUpperCase()) + line + answer + line + font.monospace(`◉ USE "CLEAR" TO RESET CONVERSATION.`);
