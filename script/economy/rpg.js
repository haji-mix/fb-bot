        ...
        try {
            ...
            switch (subcommand) {
                ...
                case "use":
                    ...
                    let itemToUse;
                    try {
                        itemToUse = await Currencies._resolveItemId(args[1]);
                    } catch (error) {
                    ...
                    if (!inventory[itemToUse] || inventory[itemToUse].quantity <= 0) {
                        const noItemText = format({
                            title: 'Use 🛠️',
                            titlePattern: `{emojis} ${UNIRedux.arrow} {word}`,
                            content: `You don't have that item! Check your inventory with: rpg inventory`
                    ...
                    if (itemDetails.name === "Health Potion" || itemDetails.name === "Mana Crystal") {
                        ...
                        await Currencies.removeItem(senderID, itemToUse, 1);
                ...
                case "trade":
                    ...
                    let tradeItemId;
                    try {
                        tradeItemId = await Currencies._resolveItemId(tradeItemName);
                    } catch (error) {
                        const noItemText = format({
                            title: 'Trade 🤝',
                            titlePattern: `{emojis} ${UNIRedux.arrow} {word}`,
                    ...
                    if (!inventory[tradeItemId] || inventory[tradeItemId].quantity < tradeQuantity) {
                        const noItemText = format({
                            title: 'Trade 🤝',
                            titlePattern: `{emojis} ${UNIRedux.arrow} {word}`,
                    ...
                    await Currencies.removeItem(senderID, tradeItemId, tradeQuantity);
                ...
                case "craft":
                    ...
                    for (const [material, qty] of Object.entries(recipe.materials)) {
                        let materialId;
                        try {
                            materialId = await Currencies._resolveItemId(material);
                        } catch (error) {
                    ...
                    for (const [material, qty] of Object.entries(recipe.materials)) {
                        ...
                        await Currencies.removeItem(senderID, materialId, qty);
                    }
                    let craftedItemId;
                    try {
                ...
                case "upgrade":
                    ...
                    await Currencies.removeBalance(senderID, upgradeCost);
                ...
                case "leaderboard":
                    const leaderboard = await Currencies.getLeaderboard(5);
                    const leaderboardText = format({
                        title: 'Leaderboard 🏆',
                        titlePattern: `{emojis} ${UNIRedux.arrow} {word}`,
                        content: `Top 5 Players:\n${leaderboard.map((user, i) => `${i + 1}. ${user.name || user.userId} - Level ${Math.floor(user.exp / 100) || 1}`).join("\n")}`
                    });
                ...
                case "heal":
                    let healItemId;
                    try {
                        healItemId = await Currencies._resolveItemId("Health Potion");
                    } catch (error) {
                    ...
                    if (!inventory[healItemId] || inventory[healItemId].quantity <= 0) {
                        const noHealText = format({
                            title: 'Heal ❤️',
                            titlePattern: `{emojis} ${UNIRedux.arrow} {word}`,
                            content: `You need a Health Potion to heal! Get one from battles or the shop.`
                    ...
                    await Currencies.removeItem(senderID, healItemId, 1);
                ...
                case "sell":
                    ...
                    let itemToSell;
                    try {
                        itemToSell = await Currencies._resolveItemId(args[1]);
                    } catch (error) {
                        const noItemText = format({
                            title: 'Sell 💸',
                            titlePattern: `{emojis} ${UNIRedux.arrow} {word}`,
                    ...
                    if (!inventory[itemToSell] || inventory[itemToSell].quantity <= 0) {
                        const noItemText = format({
                            title: 'Sell 💸',
                            titlePattern: `{emojis} ${UNIRedux.arrow} {word}`,
                            content: `You don't have that item! Check your inventory.`
                    ...
                    const sellResult = await Currencies.sellItem(senderID, itemToSell, 1, 0.8);
                    const sellText = format({
                        title: 'Sell 💸',
                        titlePattern: `{emojis} ${UNIRedux.arrow} {word}`,
                        content: `You sold a ${itemDetailsSell.name} for $${sellResult.sellPrice}! New balance: $${sellResult.newBalance.toLocaleString()}`
                    });
                    chat.reply(sellText);
                    break;
                ...
                case "donate":
                    ...
                    await Currencies.transferBalance(senderID, targetIDDonate, donateAmount);
                ...
                case "pet":
                    ...
                    let petItemId;
                    const petName = petType === "dog" ? "Dog Pet" : "Cat Pet";
                    try {
                        petItemId = await Currencies._resolveItemId(petName);
                    } catch (error) {
                        await Currencies.createItem({
                            name: petName,
                            price: petCost,
                    ...
                    await Currencies.buyItem(senderID, petItemId, 1);
