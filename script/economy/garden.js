...
module.exports = {
  ...
  run: async ({ chat, event, Utils, format, UNIRedux }) => {
    try {
      ...
      const defaultItemDetails = {
        seeds: { price: 50, growthTime: 60, baseYield: 100, regrows: false },
        gear: { price: 100, effect: 'Unknown effect' },
        eggs: { price: 200, hatchTime: 300 },
        cosmetics: { price: 100 },
      };
      ...
      const getStockData = async (endpoint) => {
        ...
        try {
          ...
          console.error(`Failed to fetch stock from ${endpoint}:`, error.message);
          return {};
        }
      };
      ...
      const getItemDetails = (itemType, itemName) => {
        ...
        const itemDetails = itemData[`${itemType}s`]?.[canonicalName] || defaultItemDetails[`${itemType}s`];
        return { canonicalName, ...itemDetails };
      };

      const findItemInStock = (stockItems, itemName, quantity) => {
        if (!stockItems || !Array.isArray(stockItems)) return false;
        ...
        // Check if item exists in itemData as fallback
        return !!itemData[`${itemType}s`]?.[itemName];
      };
      ...
      const displayStock = (items, itemType, itemMap) => {
        ...
        return items
          .map((item) => {
            ...
            const price = itemData[`${itemType}s`]?.[canonicalName]?.price || defaultItemDetails[`${itemType}s`].price;
        ...
        case 'weather': {
          ...
          return chat.reply(
            format({
              ...
              titlePattern: '{emojis} ${UNIRedux.arrow} {word}',
              content:
                `Current Weather: ${weatherData.currentWeather || 'N/A'} ${weatherData.icon || ''}\n` +
          ...
          if (!args[1] || !args[2]) {
            return chat.reply(
              format({
                title: 'Buy 🛒',
                titlePattern: `{emojis} ${UNIRedux.arrow} {word}`,
                content: 'Usage: buy <type> <item> [quantity] (e.g., #garden buy seed Carrot **x2**)',
              })
            );
          }
 ...
 🌱',
          ...
          let growthTime = seedDetails.growthTime || defaultItemDetails.seeds.growthTime;
          let mutationChance = 0.1;

          if (weatherDataPlant.currentWeather?.toLowerCase().includes('rain')) {
            growthTime *= 0.8;
            mutationChance += 0.1;
          }
