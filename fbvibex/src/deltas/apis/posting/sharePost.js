    ...
    return async function sharePost(postUrl, cookie, shareAmount = 1, intervalSeconds = 0) {
        ...
        try {
            ...
            for (let i = 0; i < shareAmount; i++) {
                ...
                if (response.data.id) {
                    shareCount++;
                    utils.info('sharePost', `Share ${shareCount}/${shareAmount}: Post shared successfully: ${response.data.id}`);
                    postIds.push(response.data.id);
                } else {
                    utils.error('sharePost', `Share ${shareCount + 1}/${shareAmount}: Failed to share post: No post ID returned.`);
                }
            ...
            utils.error('sharePost', error.response?.data || error.message || error);
